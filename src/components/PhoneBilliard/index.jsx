...
class PhoneBilliard extends React.Component {
  ...
  componentDidMount() {
    ...
    const ballRadiusY = this.ballRadius * (canvas.width / canvas.height);
    const balls = [];
    for (let i = 0; i < 10; i++) {
      const x = Math.random() * (1 - 2 * this.ballRadius) + this.ballRadius;
      const y = Math.random() * (1 - 2 * this.ballRadius) + ballRadiusY;
      balls.push(new Ball(x, y, i));
    }
    this.balls = balls;

    this.holes = [new Ball(0, 0), new Ball(1, 0), new Ball(1, 1), new Ball(0, 1)];
  ...
  handleMouseOrTouchDown(offsetX, offsetY) {
    ...
    for (const ball of this.balls) {
      ...
      if (Math.pow(offsetX - ball.x, 2) + Math.pow(offsetY - ball.y, 2) <= Math.pow(this.ballRadius, 2)) {
  ...
  handleMouseOrTouchUp(offsetX, offsetY) {
    ...
    if (this.activeBall) {
      ...
      this.activeBall.dx = Math.min(dx * this.speedFactor, this.maxSpeed);
      this.activeBall.dy = Math.min(dy * this.speedFactor, this.maxSpeed);
    }
  ...
  draw() {
    ...
    for (let hole of this.holes) {
      ...
      ctx.beginPath();
      ctx.arc(centerX, centerY, this.holeRadius * w, 0, 2 * Math.PI, false);
      ctx.fillStyle = 'gray';
      ctx.fill();
    }
    ...
    if (this.touchX && this.activeBall) {
      ...
      ctx.lineTo(this.activeBall.x * canvas.width, this.activeBall.y * canvas.height);
      ctx.stroke();
    }
  }
  ...
  update() {
    ...
    for (const ball of this.balls) {
      const ballRadiusY = this.ballRadius * (canvas.width / canvas.height);
      ball.move(this.ballRadius, ballRadiusY, this.drag, this.wallDrag, this.holes, this.holeRadius, this.addNumber);
    }
  }
    ...
    return (
      ...
      >
        <div>
          ...
          <button className="btn" onClick={this.clearNumber}>
            Start på nytt
          </button>
        </div>
...
class Ball {
  ...
  move(ballRadiusX, ballRadiusY, drag, wallDrag, holes, holeRadius, addNumberFn) {
    this.x += this.dx;
    this.y += this.dy;
    ...
    for (let hole of holes) {
      if (Math.pow(this.x - hole.x, 2) + Math.pow(this.y - hole.y, 2) <= Math.pow(holeRadius, 2)) {
        addNumberFn(this);
      }
    }
  }
