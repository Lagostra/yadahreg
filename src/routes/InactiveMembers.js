...
class InactiveMembers extends React.Component {
  ...
  componentDidMount() {
    const semesterPromise =
      !!this.props.authUser && !!this.props.authUser.permissions[PERMISSIONS.SEMESTERS_READ]
        ? this.props.firebase
            .semesters()
            .once('value')
            .then((snapshot) => {
    ...
    const eventsPromise = this.props.firebase
      ...
      .then((snapshot) => {
        ...
        let events = Object.keys(eventsObject)
          ...
          .filter((event) => event.type && event.type === 'Øvelse')
          .sort((a, b) => moment(b.date) - moment(a.date));

        const lastThree = events.slice(0, 3);
        const thisYear = moment().year();
        const semesterStart =
          moment() <= moment([thisYear, 6, 31]) ? moment([thisYear, 0, 1]) : moment([thisYear, 7, 1]);
        events = events.filter((event) => moment(event.date) > semesterStart);
        events = events.length >= 3 ? events : lastThree;

        return events;
      });

    Promise.all([memberPromise, eventsPromise, semesterPromise]).then(([members, events]) => {
      ...
      this.props.firebase
        ...
        .then((snapshot) => {
          if (!snapshot.val().active) {
            this.setState({
              inactiveMembers: this.state.inactiveMembers.filter((m) => m.id !== memberId),
            });
          }
        });
    }
  ...
  render() {
    const { inactiveMembers, loaded, modalActive, editMember, semester } = this.state;
              ...
              <tbody>
                {inactiveMembers.map((member) => (
                  <tr key={member.id}>
                    ...
                    <td>
                      <button className="btn btn-small" onClick={() => this.handleEditMember(member)}>
...
export default compose(withFirebase, withAuthorization(authCondition), withAuthUser)(InactiveMembers);
